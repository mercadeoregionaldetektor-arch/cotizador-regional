/* Detektor Cotizador Regional - Frontend Webflow
   ...
   (VERSIÓN RECONSTRUIDA: Periodos libres + Límite 2 productos)
*/
...
window.DTK_DATA = {
  countries: {
    'Costa Rica': { code: 'CR', currency: ['CRC', 'USD'], locale: 'es-CR', taxName: 'IVA', taxRates: [13, 0], phonePlaceholder: 'Ej. +506 8888 8888', cityPlaceholder: 'Ej. San José', advisorMode: 'list', agents: [
    ...
    'Panamá': { code: 'PA', currency: ['PAB', 'USD'], locale: 'es-PA', taxName: 'ITBMS', taxRates: [7, 0], phonePlaceholder: 'Ej. +507 6000 0000', cityPlaceholder: 'Ej. Ciudad de Panamá', advisorMode: 'list', agents: [
    ...
    'Guatemala': { code: 'GT', currency: ['GTQ', 'USD'], locale: 'es-GT', taxName: 'IVA', taxRates: [12, 0], phonePlaceholder: 'Ej. +502 5555 5555', cityPlaceholder: 'Ej. Ciudad de Guatemala', advisorMode: 'list', agents: [
    ...
    'Honduras': { code: 'HN', currency: ['HNL', 'USD'], locale: 'es-HN', taxName: 'ISV', taxRates: [15, 0], phonePlaceholder: 'Ej. +504 9999 9999', cityPlaceholder: 'Ej. Tegucigalpa', advisorMode: 'list', agents: [
    ...
    'El Salvador': { code: 'SV', currency: ['USD'], locale: 'es-SV', taxName: 'IVA', taxRates: [13, 0], phonePlaceholder: 'Ej. +503 7777 7777', cityPlaceholder: 'Ej. San Salvador', advisorMode: 'list', agents: [
    ...
    'Nicaragua': { code: 'NI', currency: ['NIO', 'USD'], locale: 'es-NI', taxName: 'IVA', taxRates: [15, 0], phonePlaceholder: 'Ej. +505 8888 8888', cityPlaceholder: 'Ej. Managua', advisorMode: 'list', agents: [
    ...
    'Venezuela': { code: 'VE', currency: ['VES', 'USD'], locale: 'es-VE', taxName: 'IVA', taxRates: [16, 0], phonePlaceholder: 'Ej. +58 412 000 0000', cityPlaceholder: 'Ej. Caracas', advisorMode: 'manual', agents: [], terms: {
  ...
  products: [
    { id: 'cazador', name: 'Detektor El Cazador', image: 'https://cdn.prod.website-files.com/6a73658b9794177dcea91af7/6a7b8d48e70aa94196be3df0_Productos-soluciones-detektor-cotizador%20(1).webp', imageLabel: 'IMG: RECUPERACIÓN VEHICULAR', description: 'Rastreo y localización vehicular en caso de hurto con tecnología exclusiva y equipo de reacción 24/7.', benefit: '✓ La solución más efectiva del mercado', pdfDescription: 'Tecnología especializada para localizar vehículos en caso de hurto, incluso donde otras señales no llegan.' },
    { id: 'gps', name: 'Detektor GPS', image: 'https://cdn.prod.website-files.com/6a73658b9794177dcea91af7/6a7b8d482a0eec4e41112bc8_Productos-soluciones-detektor-cotizador%20(5).webp', imageLabel: 'IMG: MAPA GPS FLOTA', description: 'Rastreo satelital de alta precisión con geocercas y reportes de ruta.', benefit: '✓ Visibilidad total de tu flota, en cualquier momento.', pdfDescription: 'Dispositivo de rastreo de alta precisión. Permite monitoreo en tiempo real, histórico de rutas y geocercas.' },
    { id: 'roadview', name: 'Roadview IA', image: 'https://cdn.prod.website-files.com/6a73658b9794177dcea91af7/6a7b8d481a47f3c932fec5b9_Productos-soluciones-detektor-cotizador%20(4).webp', imageLabel: 'IMG: CÁMARA IA VEHÍCULO', description: 'Cámaras con IA para prevención de accidentes a través del análisis en tiempo real.', benefit: '✓ Reduce accidentes con alertas en tiempo real.', pdfDescription: 'Cámara con inteligencia artificial para prevenir riesgos y mejorar la conducción.' },
    { id: 'plus', name: 'Detektor Plus', image: 'https://cdn.prod.website-files.com/6a73658b9794177dcea91af7/6a7b8d4880cacf3cb9ac7328_Productos-soluciones-detektor-cotizador%20(3).webp', imageLabel: 'IMG: SATÉLITE / FLOTA CARGA', description: 'Seguridad de sus vehículos y flotas, garantizando que podrán ser monitoreados, rastreados y localizados.', benefit: '✓ Control operativo y menores costos.', pdfDescription: 'Combina GPS + El Cazador para monitoreo diario y localización especializada en caso de hurto.' }
  ]
};
...
(() => {
  ...
  function formatMoney(value) { return `${formatNumberOnly(value)} ${currentCurrency()}`.trim(); }
  ...
  async function reserveQuoteNumber(force = false, { showError = false } = {}) {
    ...
    if (canTryApi) {
      try {
        ...
        if (els['quote-number']) els['quote-number'].value = '';
        if (showError) showNotice('El asesor está correcto, pero no fue posible generar el consecutivo. Revisa la URL de Render.', 'error');
        return '';
      }
    } else {
  ...
  function applyCountry() {
    ...
    if (!country) {
      ...
      if (els['quote-advisor-phone']) els['quote-advisor-phone'].placeholder = 'Ej. +00 000 0000';
      if ($('client-phone')) $('client-phone').placeholder = 'Ej. +00 000 0000';
      if ($('client-city')) $('client-city').placeholder = 'Ej. Ciudad';
      return;
    }
    ...
    if (countryHasAdvisorList(country)) {
      ...
      $('dtk-advisor-list').innerHTML = country.agents.map(a => `<option value="${escapeHtml(a.code)} - ${escapeHtml(a.name)}"></option>`).join('');
    } else {
    ...
    if (els['quote-advisor-phone']) els['quote-advisor-phone'].placeholder = phonePlaceholder;
    if ($('client-phone')) $('client-phone').placeholder = phonePlaceholder;
    if ($('client-city')) $('client-city').placeholder = cityPlaceholder;
  ...
  function appendRow(name = '', qty = 1, price = 0, discount = 0, productId = '', period = 'Mensual') {
    ...
    // AQUÍ INYECTAMOS LA CELDA "PERIODO" PARA RECONSTRUIR EL DISEÑO DE LA TABLA
    tr.innerHTML = `
  ...
  function addCatalogProduct(productId) {
    ...
    // Límite modificado a 2 ocurrencias
  ...
  function rowData() {
    return [...(els['dtk-calc-tbody']?.querySelectorAll('tr[data-row]') || [])].map(row => {
      ...
      return {
        ...
        period, qty, price, discount, subtotal
      };
    });
  }
  ...
  function calculateAll() {
    ...
    if (els['val-tax']?.dataset.mode === 'auto') els['val-tax'].textContent = formatMoney(tax);
  ...
  async function validateForm({ requireFinalNumber = false } = {}) {
    ...
    for (const id of requiredIds) {
      ...
      if (!el || !value) { el?.classList.add('dtk-error'); valid = false; } 
      else if (id === 'client-email' && !emailRegex.test(value)) { el?.classList.add('dtk-error'); valid = false; }
    }

    const advEmailEl = $('quote-advisor-email');
    const advEmailVal = String(advEmailEl?.value || '').trim();
    if (advEmailVal && !emailRegex.test(advEmailVal)) { advEmailEl.classList.add('dtk-error'); valid = false; }

    const country = getCountry();
    if (countryHasAdvisorList(country)) { if (!els['quote-advisor-select'].value) { els['quote-advisor-select'].classList.add('dtk-error'); valid = false; } } 
    else if (country) { if (!els['quote-advisor-manual'].value.trim()) { els['quote-advisor-manual'].classList.add('dtk-error'); valid = false; } }
    ...
    if (!number) {
      if (requireFinalNumber) { els['quote-number']?.classList.add('dtk-error'); els['quote-number']?.scrollIntoView({ behavior:'smooth', block:'center' }); return false; }
  ...
  function populateCountryContact() {
    ...
    web.textContent = contact.web.replace(/^https?:\/\//, '').replace(/\/$/, '');
  }
  ...
  function populatePreview() {
    ...
    const taxRate = currentTaxRate();
    setText('prev-tax-percent', `(${taxRate}%)`, '');
    ...
    const chosen = DATA.products.map(product => ({ productId: product.id, name: product.name }));
    ...
    if (solutions) {
      ...
      if (!chosen.length) {
        ...
        if (solutionsTitle?.classList?.contains('pdf-section-title')) solutionsTitle.style.display = 'none';
      } else {
        solutions.style.display = '';
        if (solutionsTitle?.classList?.contains('pdf-section-title')) solutionsTitle.style.display = '';
        ...
        solutions.innerHTML = chosen.map((item, i) => {
          ...
          const bullets = product.id === 'gps' ? ['● Monitoreo y recorridos', '● Geocercas y alertas', '● Reportes de operación'] : ['● Tecnología especializada', '● Configuración según operación', '● Respaldo Detektor'];
          const isSelected = selectedProductIds.includes(product.id);

          return `<div class="pdf-solution ${isSelected ? 'is-selected' : ''}">
            <div class="pdf-solution-img" style="overflow:hidden;">${productImageHtml(product, 'pdf')}</div>
    ...
    const termsLines = [];
    if (gv('terms-payment')) termsLines.push(`Condiciones de pago: ${gv('terms-payment')}`);
    if (gv('terms-installation')) termsLines.push(`Instalación y entrega: ${gv('terms-installation')}`);
    if (gv('terms-validity')) termsLines.push(`Vigencia: ${gv('terms-validity')}`);
    if (gv('terms-warranty')) termsLines.push(`Garantía: ${gv('terms-warranty')}`);
    if (gv('terms-extra')) {
  ...
  function restorePdfDynamicLayout() {
    ...
    if (inner && pdfPage2BaseHtml) inner.innerHTML = pdfPage2BaseHtml;
  }
  ...
  function splitSolutionsGroup(group, state) {
    ...
    if (!title || !list || !items.length) { state.inner.appendChild(group); return; }

    let firstChunk = true, chunk = null, chunkList = null;

    const newChunk = () => {
      chunk = document.createElement('div');
      chunk.className = 'pdf-flow-group pdf-flow-solutions';
      chunk.appendChild(firstChunk ? title.cloneNode(true) : continuationTitle(title));
    ...
    items.forEach(item => {
      ...
      if (!currentPageFits(state.page)) {
        ...
        if (!chunkList.children.length) { chunkList.appendChild(item); return; }
  ...
  function splitEconomicGroup(group, state) {
    ...
    if (!title || !sourceTable) { state.inner.appendChild(group); return; }

    let firstChunk = true, chunk = null, shell = null;
    ...
    rows.forEach(row => {
      ...
      if (!currentPageFits(state.page)) {
        ...
        if (!shell.tbody.children.length) { shell.tbody.appendChild(row); return; }
        startNewPdfPage(state);
        newChunk();
        shell.tbody.appendChild(row);
      }
  ...
  function markFinalPdfPage() {
    ...
    if (lastPage) lastPage.classList.add('pdf-final-page');
  }
  ...
  function rebuildPageNavigation() {
    ...
    if (!dots) return;
    dots.innerHTML = pages.map((page, index) => `<button class="dtk-nav-dot ${index === 0 ? 'active' : ''}" data-target="${page.id}">${index + 1}</button>`).join('');
    dots.querySelectorAll('.dtk-nav-dot').forEach(dot => { dot.addEventListener('click', () => { scrollToPage(dot.dataset.target, dot); }); });
  }
  ...
  function paginatePdfDynamically() {
    ...
    const groups = [...sourceInner.querySelectorAll(':scope > .pdf-flow-group')];
    sourceInner.innerHTML = '';
    const state = { page: page2, inner: sourceInner, pageIndex: 2 };

    groups.forEach(group => {
      const type = group.dataset.flowGroup;
      if (type === 'solutions' && group.querySelector('.pdf-solutions')?.style.display === 'none') return;

      if (type === 'solutions') {
        state.inner.appendChild(group);
        if (!currentPageFits(state.page)) {
          state.inner.removeChild(group);
          if (state.inner.children.length) startNewPdfPage(state);
          state.inner.appendChild(group);
          if (!currentPageFits(state.page)) { state.inner.removeChild(group); splitSolutionsGroup(group, state); }
        }
        return;
      }
      ...
      if (type === 'economic') {
        ...
        if (!currentPageFits(state.page)) {
          ...
          if (state.inner.children.length) startNewPdfPage(state);
          state.inner.appendChild(group);
          if (!currentPageFits(state.page)) { state.inner.removeChild(group); splitEconomicGroup(group, state); }
        }
        return;
      }
    ...
    [...document.querySelectorAll('.pdf-generated-page')].forEach(page => {
      ...
      if (pageInner && !pageInner.children.length) page.remove();
    });
  ...
  async function openPreview() {
    ...
    requestAnimationFrame(() => { els['modal-scroll-area'].scrollTop = 0; updateActiveDot(); });
  }
  ...
  function saveDraft() {
    ...
    const draft = {
      quoteDate: els['quote-date']?.value || '', quoteNumber: els['quote-number']?.value || '',
      reservedKeyState: reservedKey, reservedNumberState: reservedNumber,
      quoteCountry: els['quote-country']?.value || '', quoteAdvisorSelect: els['quote-advisor-select']?.value || '',
      quoteAdvisorManual: els['quote-advisor-manual']?.value || '', quoteAdvisorPhone: els['quote-advisor-phone']?.value || '',
      quoteAdvisorEmail: els['quote-advisor-email']?.value || '', quoteObs: $('quote-obs')?.value || '',
      clientName: $('client-name')?.value \vert{}\vert{} '', clientCompany:$('client-company')?.value || '',
      clientRole: $('client-role')?.value \vert{}\vert{} '', clientEmail:$('client-email')?.value || '',
      clientPhone: $('client-phone')?.value \vert{}\vert{} '', clientCity:$('client-city')?.value || '',
      termsInst: $('terms-installation')?.value \vert{}\vert{} '', termsPay:$('terms-payment')?.value || '',
      termsVal: $('terms-validity')?.value \vert{}\vert{} '', termsWarr:$('terms-warranty')?.value || '',
      termsExtra: $('terms-extra')?.value || '', currency: els['currency-select']?.value || '',
      taxSelect: els['select-tax']?.value || '', taxManual: els['input-tax-manual']?.value || '',
      rows: rowData().map(r => ({ name: r.name, qty: r.qty, period: r.period, price: r.price, discount: r.discount, productId: r.productId })),
      modes: { subtotal: els['val-subtotal']?.dataset.mode || 'auto', tax: els['val-tax']?.dataset.mode || 'auto', total: els['val-total']?.dataset.mode || 'auto' },
      manualValues: { subtotal: els['val-subtotal']?.textContent || '0', tax: els['val-tax']?.textContent || '0', total: els['val-total']?.textContent || '0' }
    };
  ...
  function loadDraft() {
    try {
      ...
      if (draft.quoteCountry) { els['quote-country'].value = draft.quoteCountry; applyCountry(); }
      ...
      setVal('quote-date', draft.quoteDate); setVal('quote-number', draft.quoteNumber);
      if (reservedNumber) els['quote-number'].value = reservedNumber;
      
      setVal('quote-advisor-select', draft.quoteAdvisorSelect);
      setVal('quote-advisor-manual', draft.quoteAdvisorManual);
      if (draft.quoteCountry && countryHasAdvisorList(getCountry())) onAdvisorSelect(); else syncManualAdvisorCode();

      setVal('quote-advisor-phone', draft.quoteAdvisorPhone); setVal('quote-advisor-email', draft.quoteAdvisorEmail);
      setVal('quote-obs', draft.quoteObs); setVal('client-name', draft.clientName);
      setVal('client-company', draft.clientCompany); setVal('client-role', draft.clientRole);
      setVal('client-email', draft.clientEmail); setVal('client-phone', draft.clientPhone);
      setVal('client-city', draft.clientCity); setVal('terms-installation', draft.termsInst);
      setVal('terms-payment', draft.termsPay); setVal('terms-validity', draft.termsVal);
      setVal('terms-warranty', draft.termsWarr); setVal('terms-extra', draft.termsExtra);
  ...
  window.DTK_DEBUG_ADVISORS = function() {
    ...
    return Object.fromEntries(names.map(name => {
      ...
      return [name, { mode: countryHasAdvisorList(c) ? 'list' : 'manual', advisors: c?.agents?.length || 0 }];
    }));
  };
  ...
  function init() {
    ...
    const advisorCounts = Object.fromEntries(listCountries.map(name => [name, DATA.countries[name]?.agents?.length || 0]));
    ...
    if (page2Inner) pdfPage2BaseHtml = page2Inner.innerHTML;
    ...
    if (!loadDraft()) {
      ...
      setTimeout(() => showNotice('Borrador previo cargado con éxito.', 'success'), 500);
    }
