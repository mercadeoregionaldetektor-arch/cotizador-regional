/* =========================================================================
   DETEKTOR — COTIZADOR MANUAL
   Lógica de la aplicación (front-end logic, hospedada en GitHub).
   Se referencia desde el Embed de Webflow vía <script src="...">.
   ========================================================================= */
document.addEventListener('DOMContentLoaded', () => {
  ...
  const STORAGE_KEY_FORM     = 'dtk_quote_form_data_v12';
  const STORAGE_KEY_ROWS     = 'dtk_quote_table_rows_v12';
  const STORAGE_KEY_COUNTERS = 'dtk_quote_counters_v12';

  /* ----------------------------------------------------------------------
     DATOS POR PAÍS
     - agents: []  ->  el país NO tiene lista de asesores (captura manual)
     - agents: [...] -> el país SÍ tiene lista desplegable de asesores
     Países CON lista: Honduras, El Salvador, Nicaragua, Costa Rica, Guatemala
     Países SIN lista: Colombia, Panamá, Venezuela (captura manual)
     -------------------------------------------------------------------- */
  const countryData = {
    'Colombia': {
      code: 'CO',
    ...
    'Venezuela': {
      ...
      agents: [],
      terms: {
        installation: 'Pendiente de actualización VE',
        payment: 'Pendiente de actualización VE',
        validity: 'Pendiente de actualización VE',
        warranty: 'Pendiente de actualización VE',
        extra: 'Pendiente de actualización VE'
      }
    }
  };
  ...
  /* ----------------------------------------------------------------------
     UTILIDADES
     -------------------------------------------------------------------- */
  const sanitizeCode = (str) => (str || '').toString().trim().toUpperCase().replace(/[^A-Z0-9-]/g, '');

  const getCountryData = () => {
    const c = document.getElementById('quote-country')?.value;
    return countryData[c] || null;
  };

  const hasAdvisorList = (data) => !!(data && data.agents && data.agents.length > 0);

  const getAdvisorCode = () => {
    const data = getCountryData();
    if (!data) return '';
    if (hasAdvisorList(data)) {
      const sel = document.getElementById('quote-advisor');
      if (!sel || sel.selectedIndex < 0) return '';
      const opt = sel.options[sel.selectedIndex];
      return opt ? sanitizeCode(opt.getAttribute('data-code')) : '';
    }
    const codeInput = document.getElementById('quote-advisor-code');
    return codeInput ? sanitizeCode(codeInput.value) : '';
  };

  const updateQuoteNumber = () => {
    const data = getCountryData();
    const quoteNumberEl = document.getElementById('quote-number');
    if (!data || !quoteNumberEl) { if (quoteNumberEl) quoteNumberEl.value = ''; return; }
    ...
    let quoteNum = `DET-${baseCode}-${year}-${numStr}`;
    if (advisorCode) quoteNum += `-${advisorCode}`;
    quoteNumberEl.value = quoteNum;
  };
  ...
  /* ----------------------------------------------------------------------
     CAMPO DINÁMICO DE ASESOR COMERCIAL
     -------------------------------------------------------------------- */
  const renderAdvisorField = (data) => {
    const container = document.getElementById('advisor-field-container');
    const wrapCode = document.getElementById('wrap-advisor-code');
    const codeInput = document.getElementById('quote-advisor-code');
    if (!container) return;

    if (hasAdvisorList(data)) {
      container.innerHTML = '<select id="quote-advisor" class="dtk-select dtk-persist dtk-required"><option value="" disabled selected>Seleccione un asesor</option></select>';
      const sel = document.getElementById('quote-advisor');
      data.agents.forEach(agent => {
        sel.innerHTML += `<option value="${agent.name}" data-code="${agent.code}">${agent.code} - ${agent.name}</option>`;
      });
      sel.addEventListener('change', () => { updateQuoteNumber(); saveState(); calculateAll(); });
      sel.addEventListener('change', function () { this.classList.remove('dtk-error'); });

      if (wrapCode) wrapCode.style.display = 'none';
      if (codeInput) { codeInput.classList.remove('dtk-required'); codeInput.value = ''; }
    } else {
      container.innerHTML = '<input type="text" id="quote-advisor" class="dtk-input dtk-persist dtk-required" placeholder="Ej: Nombre del asesor comercial">';
      const inp = document.getElementById('quote-advisor');
      inp.addEventListener('input', () => { inp.classList.remove('dtk-error'); updateQuoteNumber(); saveState(); });

      if (wrapCode) wrapCode.style.display = 'flex';
      if (codeInput) {
        codeInput.classList.add('dtk-required');
        codeInput.oninput = null;
        codeInput.addEventListener('input', function () {
          this.classList.remove('dtk-error');
          updateQuoteNumber();
          saveState();
        });
      }
  ...
  document.getElementById('quote-country')?.addEventListener('change', function (e) {
    ...
    renderAdvisorField(data);

    const curSelect = document.getElementById('currency-select');
    if (curSelect) {
      curSelect.innerHTML = '';
      data.currency.forEach(cur => { curSelect.innerHTML += `<option value="${cur}">${cur}</option>`; });
    }
    ...
    if (taxSelect) {
      taxSelect.innerHTML = '';
      data.taxRates.forEach(rate => { taxSelect.innerHTML += `<option value="${rate}">${rate}%</option>`; });
    }
  ...
  const appendRow = (name, price, qty = 1, desc = 0) => {
    ...
    tr.innerHTML = `
      <td><input type="text" class="dtk-input dtk-prod-name dtk-required" value="${name}" placeholder="Ej: Detektor GPS"></td>
      <td><input type="number" class="dtk-input dtk-qty" value="${qty}" min="0" step="any" placeholder="1"></td>
      <td><input type="number" class="dtk-input dtk-price" value="${price}" step="any" placeholder="0"></td>
      <td><input type="number" class="dtk-input dtk-desc" value="${desc}" min="0" max="100" step="any" placeholder="0"></td>
      <td style="text-align:right; font-weight:700;" class="dtk-row-subtotal">0</td>
      <td style="text-align:right;"><button type="button" class="dtk-remove-item">×</button></td>
    `;
  ...
  const validateForm = () => {
    ...
    document.querySelectorAll('.dtk-required').forEach(field => {
      if (!field.value || !field.value.toString().trim()) { field.classList.add('dtk-error'); isValid = false; }
    ...
    let anyProductName = false;
    tbody?.querySelectorAll('.dtk-prod-name').forEach(el => { if (el.value.trim()) anyProductName = true; });
    if (!anyProductName) {
      showNotification('Agrega al menos un producto con nombre a la cotización.', 'error');
      return false;
    }
    if (!isValid) {
  ...
  document.querySelectorAll('.dtk-btn-add').forEach(btn => { btn.addEventListener('click', (e) => { appendRow(e.target.getAttribute('data-name'), 0); calculateAll(); }); });
  ...
  document.getElementById('quote-date')?.addEventListener('change', saveState);
  ...
  document.getElementById('btn-confirm-yes')?.addEventListener('click', () => {
    const currentData = getCountryData();
    if (currentData) {
      let counters = JSON.parse(localStorage.getItem(STORAGE_KEY_COUNTERS)) || {};
      counters[currentData.code] = (counters[currentData.code] || 1) + 1;
    ...
    const advisorContainer = document.getElementById('advisor-field-container');
    if (advisorContainer) advisorContainer.innerHTML = '<select id="quote-advisor" class="dtk-select dtk-persist dtk-required"><option value="" disabled selected>Seleccione el país primero</option></select>';
    const wrapCode = document.getElementById('wrap-advisor-code');
    if (wrapCode) wrapCode.style.display = 'none';
    ...
    if (confirmBtnRow) confirmBtnRow.style.display = 'none';
    if (mainBtnRow) mainBtnRow.style.display = 'flex';
    showNotification('Formulario limpiado. Nuevo lienzo listo.', 'success');
  });
  ...
  const populateModal = () => {
    ...
    const data = getCountryData();
    let advisorDisplayName = getVal('quote-advisor');
    if (data && hasAdvisorList(data)) {
      const advSelect = document.getElementById('quote-advisor');
      advisorDisplayName = advSelect && advSelect.selectedIndex >= 0 ? advSelect.options[advSelect.selectedIndex].text : '';
    }
    setText('prev-quote-advisor', advisorDisplayName);
    ...
    const advisorNameForBox = data && hasAdvisorList(data) ? advisorDisplayName.replace(/^.*-\s*/, '') : getVal('quote-advisor');
    setText('prev-adv-name-box', advisorNameForBox);
  ...
  document.getElementById('btn-preview-pdf')?.addEventListener('click', () => {
    ...
    if (modal) {
      ...
      document.querySelector('.dtk-nav-dot[data-target="pdf-page-1"]')?.classList.add('active');
  ...
  /* Navegación 1 / 2 / 3 con scroll suave dentro del modal */
  document.querySelectorAll('.dtk-nav-dot').forEach(dot => {
    dot.addEventListener('click', function (e) {
      ...
      if (targetPage && modalScrollArea) {
        ...
        modalScrollArea.scrollTo({ top: scrollTopPos, behavior: 'smooth' });
      }
    });
  });

  /* Sincroniza el punto activo cuando el usuario hace scroll manual dentro del modal */
  const modalScrollArea = document.getElementById('modal-scroll-area');
  if (modalScrollArea && 'IntersectionObserver' in window) {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          document.querySelectorAll('.dtk-nav-dot').forEach(d => d.classList.remove('active'));
          document.querySelector(`.dtk-nav-dot[data-target="${entry.target.id}"]`)?.classList.add('active');
        }
      });
    }, { root: modalScrollArea, threshold: 0.55 });
    ['pdf-page-1', 'pdf-page-2', 'pdf-page-3'].forEach(id => {
      const el = document.getElementById(id);
