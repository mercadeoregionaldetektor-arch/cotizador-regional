(() => {
  ...
  async function reserveQuoteNumber(force = false) {
    ...
    const canTryApi = location.protocol !== 'file:';
    try {
      if (canTryApi) {
      ...
      console.warn('Backend no disponible; se usa consecutivo local.', error);
    }

    if (!quoteNumber) quoteNumber = localReserve(country.code, advisorCode, year);
    reservedKey = key;
    reservedNumber = quoteNumber;
    if (els['quote-number']) els['quote-number'].value = quoteNumber;
    return quoteNumber;
  }
  ...
  async function validateForm() {
    ...
    if (valid) {
      const number = await reserveQuoteNumber(false);
      if (!number) {
        els['quote-advisor-code'].classList.add('dtk-error');
        valid = false;
      }
    }
  ...
  function wireEvents() {
    ...
    els['quote-advisor-code'].addEventListener('blur', () => reserveQuoteNumber(false));
